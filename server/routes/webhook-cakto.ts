...
// Rota principal do webhook Cakto
router.post('/', async (req, res) => {
    console.log('🔥 WEBHOOK CAKTO CHAMADO!');
    console.log('🕒 Timestamp:', new Date().toISOString());
    console.log('📨 Headers:', JSON.stringify(req.headers, null, 2));

    const payload = req.body;
    const eventType = payload?.type || payload?.event || payload?.event_type || 'SEM_EVENTO';
    console.log('📩 Webhook Cakto recebido:', eventType);
    console.log('📋 Payload completo:', JSON.stringify(payload, null, 2));

    // Salvar payload para diagnóstico futuro
    try {
        await pool.query('INSERT INTO webhook_logs (provider, payload) VALUES ($1, $2)', ['cakto', payload]);
    } catch (logErr) {
    ...
    if (isApprovedStatus(status)) {
        try {
            // Extração de dados (cobrindo possíveis estruturas de payload)
            const buyerData = payload?.client || payload?.customer || payload?.buyer || payload?.data?.buyer || payload?.data?.customer || {};
            const productData = payload?.product || payload?.data?.product || {};
            const saleData = payload?.sale || payload?.transaction || payload?.data?.transaction || payload || {};
            const contractData = payload?.contract || payload?.subscription || payload?.data?.subscription || {};

            const email = (buyerData?.email || payload?.email || '')?.toLowerCase().trim();
            const name = buyerData?.name || buyerData?.full_name || payload?.name || 'Usuário Cakto';
            ...
            const transactionId = saleData?.transaction || saleData?.id || saleData?.order_id || `CK_${Date.now()}`;

            console.log(`📞 Telefone capturado (Cakto): ${telefone || 'não informado'}`);
            console.log(`💳 Transação ID capturada: ${transactionId}`);

            // Captura dados do plano
            const planName = productData?.name || saleData?.product_name || 'Plano Premium (Cakto)';
            ...
            console.log(`🎯 Dados da compra Cakto: Email=${email}, Plano=${planName}, Tipo=${planType}, Transação=${transactionId}`);
            ...
            if (!email) {
                console.log('❌ Email não encontrado no payload Cakto');
          ...
          VALUES ($1, $2, $3, $4, 'premium', $5, $6, true, NOW(), false, 'Cliente Cakto', 'cakto')
        `, [email, username, hashedPassword, telefone, planType, endDate]);

                console.log(`✅ Novo usuário criado: ${name} (${email}) - Plano Cakto: ${planName}`);
                ...
                try {
                    ...
                    console.log('⚠️ Erro ao enviar emails de boas-vindas:', emailError);
                }
            } else {
          ...
          UPDATE users SET 
            ...
            telefone = $4,
            active = true,
            origem_assinatura = 'cakto'
          WHERE email = $1
        `, [email, planType, endDate, telefone]);

                console.log(`✅ Usuário atualizado para premium: ${name} (${email}) - Plano Cakto: ${planName}`);
            }
            ...
            try {
                ...
                `, [userId, email, 'cakto', transactionId, planPrice, planCurrency, 'approved', planName]);

            } catch (dbErr) {
                console.error('❌ Erro ao registrar transação (Cakto):', dbErr);
            }
            ...
            try {
                ...
                console.log('⚠️ Erro ao enviar email de compra:', emailError);
            }
            ...
            try {
                await UTMifyService.sendConversion({
                    ...
                    productId: productData?.id?.toString() || 'cakto_product'
                });
            } catch (utmifyErr) {
                console.error('❌ Erro ao enviar conversão Cakto para UTMify:', utmifyErr);
            }

            return res.status(200).json({ success: true, message: 'Processado com sucesso (Cakto)' });
        } catch (err: any) {
            console.error('❌ Erro ao processar compra Cakto:', err);
    ...
    if (isCancellationStatus(status)) {
        try {
            const buyerData = payload?.client || payload?.customer || payload?.buyer || payload?.data?.buyer || payload?.data?.client || payload?.data?.customer || {};
            const email = (buyerData?.email || payload?.email || '')?.toLowerCase().trim();

            if (!email) {
                console.log('❌ Email não encontrado no payload de cancelamento (Cakto)');
                return res.status(400).json({ error: 'Email não encontrado' });
            }

            console.log(`🔄 Processando cancelamento (Cakto) para: ${email}, status: ${status}`);
      ...
      `, [email]);
            ...
            console.error('❌ Erro ao processar cancelamento Cakto:', err);
    ...
    return res.status(200).json({ success: true, message: 'Webhook Cakto recebido mas ignorado', status });
});
