...
router.post('/', async (req, res) => {
  console.log('🔥 WEBHOOK HOTMART FIXED CHAMADO!');
  console.log('🕒 Timestamp:', new Date().toISOString());
  console.log('📨 Headers:', JSON.stringify(req.headers, null, 2));
  
  const payload = req.body;
  console.log('📩 Webhook recebido:', payload?.event || 'SEM EVENTO');
  ...
  if (isValidPurchase(payload)) {
    try {
      ...
      const now = new Date();
      // Calcula data de expiração baseada no tipo de plano
      ...
      if (existingUser.rowCount === 0) {
        ...
        console.log(`✅ Novo usuário criado: ${name} (${email}) - Plano: ${planName}`);
      } else {
        ...
        console.log(`✅ Usuário atualizado para premium: ${name} (${email}) - Plano: ${planName}`);
      }
      ...
      if (existingSubscription.rowCount === 0) {
        ...
        `, [
          ...
          payload.data?.product?.id || planType, // hotmart_plan_id
          planName, // hotmart_plan_name  
          payload.data?.product?.price || payload.data?.purchase?.price || 0, // hotmart_plan_price
          payload.data?.product?.currency || 'BRL' // hotmart_currency
        ]);
...
router.get('/test', (req, res) => {
  console.log('🧪 Endpoint de teste do webhook FIXED chamado');
  res.json({ 
    status: 'OK', 
    message: 'Webhook FIXED está funcionando',
