...
- July 10, 2025. Optimized home feed display: limited main feed to maximum 8 posts per column to reduce scrolling before "Ver todas as artes" button. Feed ensures balanced distribution across all columns to prevent empty spaces, maintaining consistent masonry layout on mobile (2 cols × 8 posts each) and desktop (5 cols × 8 posts each). Fixed mobile grid imbalance by ensuring posts are divisible by column count to avoid uneven distribution.
