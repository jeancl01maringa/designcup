...
- July 21, 2025. Completed login/register page redesign with centralized modal layout: implemented background with aesthetic images matching user mockup, added centered white modal with official logo from admin panel at top, removed all lateral promotional content, applied brand gradient colors (#F84930 to #F8A441) to buttons, and created clean authentication interface with proper routing for /auth/login and /auth/register paths. Design now matches user specifications with logo positioning, background overlay, and modal centering.
- July 21, 2025. Enhanced category previews and main feed display: modified CategorySection to show only "Cartaz" format in previews to eliminate content duplication. Implemented comprehensive feed filtering system with "Em alta" (randomized by date groups), "Recentes" (newest first), and "Antigos" (oldest first) options. Added sophisticated anti-duplication logic prioritizing Cartaz format over Stories to prevent same content appearing multiple times in single feed view. Feed now displays unique arts while maintaining chronological order and category mixing.
- July 23, 2025. Updated Brevo email integration to use Template ID 1: replaced custom HTML emails with Brevo template system for automated user communications. Webhook now sends welcome emails and purchase confirmations using Template ID 1 with dynamic parameters (nome, plano, valor). System maintains Lista ID 1 for contact management while leveraging professional Brevo templates for consistent branding and improved deliverability.
- July 29, 2025. Fixed Brevo Template ID 1 parameter mapping: corrected webhook parameters from uppercase (NOME, EMAIL, PLANO, VALOR) to lowercase (nome, email, plano, valor) to match Brevo template syntax {{params.email}}. Template integration now works correctly for both new user creation and existing user purchase confirmation emails. Identified webhook limitation where only specific payment plans trigger automation - other Hotmart links may require manual user upgrade.
- July 29, 2025. Completed critical Hotmart webhook data extraction bug fix: replaced hardcoded "mensal" plan types and fixed dates with real Hotmart subscription data. Webhook now extracts authentic plan duration (anual/mensal) from plan names, captures real start dates from approved_date, uses date_next_charge for accurate expiration dates, and extracts correct pricing from subscription → purchase → product hierarchy. All subscription management now uses actual Hotmart plan data instead of fixed 30-day periods, ensuring accurate user access control and billing cycles.
