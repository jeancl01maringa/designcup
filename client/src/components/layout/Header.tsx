            ...
            <div className="px-6 py-6 space-y-6">
                ...
                {navItems.map((item) => {
                  ...
                  if (item.path === "whatsapp") {
                    return (
                      <div
                        key={item.name}
                        className="flex items-center px-4 py-3 text-base font-medium text-muted-foreground transition-all duration-200 hover:bg-muted/80 rounded-xl cursor-pointer group"
                        onClick={() => {
                          if (whatsappUrl) {
                      ...
                      >
                        <div className="w-10 h-10 bg-card border border-border rounded-xl flex items-center justify-center mr-4 shadow-sm group-hover:shadow-md transition-all duration-200">
                          {getIcon(item.name)}
                        </div>
                        <span className="flex-1 text-foreground">{item.name}</span>
                        <svg className="w-4 h-4 text-gray-400 group-hover:text-muted-foreground transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              {user && (!user.tipo || user.tipo === 'free') && (
                <div className="border-t border-border pt-4">
                  ...
                  >
                    <div className="w-10 h-10 bg-card/20 border border-white/20 rounded-xl flex items-center justify-center mr-4 shadow-sm group-hover:shadow-md transition-all duration-200">
            ...
            {showScrollSearchBar ? (
              <form onSubmit={handleScrollSearch} className="relative flex items-center max-w-xl w-full">
                <input
                  ...
                  className="w-full py-4 px-6 pr-48 rounded-xl border border-border bg-card/90 backdrop-blur-sm focus:outline-none focus:ring-2 focus:ring-[#AA5E2F]/40 focus:border-[#AA5E2F] transition-all font-sans text-base placeholder:text-gray-400"
                />
                ...
                <div className="absolute right-16 top-1/2 -translate-y-1/2 border-l border-border pl-4">
                  <div className="relative" ref={dropdownRef}>
                    <button
                      ...
                      className="flex items-center justify-between text-sm font-medium px-3 py-2 min-w-[110px] text-muted-foreground bg-transparent hover:bg-muted rounded-md transition-colors duration-150 focus:outline-none"
                    >
                    ...
                    {showFormatDropdown && (
                      <div className="absolute right-0 mt-2 w-44 bg-card border border-border rounded-lg shadow-xl z-[9999]">
                        <div className="py-2">
                          {formats.map(format => (
                            <button
                              key={format.id}
                              ...
                              className={`w-full text-left px-4 py-3 text-sm transition-colors duration-150 flex items-center gap-3
                                ${format.id === selectedFormat
                                  ? 'bg-muted text-foreground font-medium border-l-2 border-gray-400'
